...
export function CalendarToolbar({ date, view, members, scopeId, onScope, onDate, onView, onAdd }: Props) {
  ...
  return <div className="calendar-toolbar">
    ...
    <div className="calendar-controls"><button type="button" className="secondary-button" onClick={() => onDate(new Date(), 0)}>Today</button><button type="button" className="arrow-button" onClick={() => move(-1)} aria-label="Previous">‹</button><h2>{title}</h2><button type="button" className="arrow-button" onClick={() => move(1)} aria-label="Next">›</button><div className={`segmented view-${view}`} aria-label="Calendar view">{(['month', 'week', 'day'] as CalendarView[]).map((item) => <button type="button" aria-pressed={view === item} className={view === item ? 'active' : ''} key={item} onClick={() => onView(item)}>{item}</button>)}</div><AddMenu onSelect={onAdd} /></div>
  </div>;
}
