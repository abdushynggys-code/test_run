...
interface Props { date: Date; view: CalendarView; members: FamilyMember[]; scopeId: string | null; onScope: (id: string | null) => void; onDate: (date: Date) => void; onView: (view: CalendarView) => void; onAdd: () => void; }
...
export function CalendarToolbar({ date, view, members, scopeId, onScope, onDate, onView, onAdd }: Props) {
  ...
  return <div className="calendar-toolbar">
    ...
    <div className="calendar-controls"><button className="secondary-button" onClick={() => onDate(new Date())}>Today</button><button className="arrow-button" onClick={() => move(-1)} aria-label="Previous">‹</button><h2>{title}</h2><button className="arrow-button" onClick={() => move(1)} aria-label="Next">›</button><div className="segmented" aria-label="Calendar view">{(['month', 'week', 'day'] as CalendarView[]).map((item) => <button className={view === item ? 'active' : ''} key={item} onClick={() => onView(item)}>{item}</button>)}</div><button className="primary-button add-event-desktop" onClick={onAdd}>＋ Add event</button></div>
  </div>;
}
