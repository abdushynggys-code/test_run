...
export function EventForm({ members, initialDate, initialEndDate, initialMemberId, onSubmit, onCancel }: Props) {
  ...
  return <form className="form-grid" onSubmit={submit}>
    ...
    <label>Family member<select value={memberId} onChange={(event) => setMemberId(event.target.value)}>{members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label><label>Custom color<div className="event-color-field"><input type="color" value={color || '#6157e5'} onChange={(event) => setColor(event.target.value)} /><button type="button" onClick={() => setColor('')}>Use profile color</button></div></label>
